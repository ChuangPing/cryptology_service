...
func Setup(configFile string, info *conf.SdkEnvInfo) (*fabsdk.FabricSDK, error) {
	// Create SDK setup for the integration tests
	sdk, err := fabsdk.New(config.FromFile(configFile))
	fmt.Println("config.FromFile1111111111111111", config.FromFile(configFile))
	if err != nil {
		logrus.Error("fabsdk new failed,err:", err)
		return nil, err
	}
	...
	for _, org := range info.Orgs {
		...
		if err != nil {
			logrus.Error("mspClient new failed, err:", err)
			return nil, err
		}
		...
		if err != nil {
			logrus.Error("\"根据指定的资源管理客户端Context创建通道管理客户端失败: %v\", err", err)
			return nil, err
		}
...
func CreateAndJoinChannel(info *conf.SdkEnvInfo) error {
	fmt.Println(">> 开始创建通道11......")
	if len(info.Orgs) == 0 {
...
func createChannel(signIDs []msp.SigningIdentity, info *conf.SdkEnvInfo) error {
...
func CreateCCLifecycle(info *conf.SdkEnvInfo, sequence int64, upgrade bool, sdk *fabsdk.FabricSDK) error {
	if len(info.Orgs) == 0 {
...
func installCC(label string, ccPkg []byte, orgs []*conf.OrgInfo) error {
...
func getInstalledCCPackage(packageID string, org *conf.OrgInfo) error {
	// use org1
...
func queryInstalled(packageID string, org *conf.OrgInfo) error {
	orgPeers, err := DiscoverLocalPeers(*org.OrgAdminClientContext, 1)
	if err != nil {
		return fmt.Errorf("DiscoverLocalPeers error: %v", err)
	}
...
func approveCC(packageID string, ccName, ccVersion string, sequence int64, channelID string, orgs []*conf.OrgInfo, ordererEndpoint string) error {
	mspIDs := []string{}
	for _, org := range orgs {
...
func queryApprovedCC(ccName string, sequence int64, channelID string, orgs []*conf.OrgInfo) error {
...
func checkCCCommitReadiness(packageID string, ccName, ccVersion string, sequence int64, channelID string, orgs []*conf.OrgInfo) error {
	mspIds := []string{}
	for _, org := range orgs {
		mspIds = append(mspIds, org.OrgMspId)
	}
...
func commitCC(ccName, ccVersion string, sequence int64, channelID string, orgs []*conf.OrgInfo, ordererEndpoint string) error {
	mspIDs := []string{}
	for _, org := range orgs {
		mspIDs = append(mspIDs, org.OrgMspId)
	}
...
func queryCommittedCC(ccName string, channelID string, sequence int64, orgs []*conf.OrgInfo) error {
...
func initCC(ccName string, upgrade bool, channelID string, org *conf.OrgInfo, sdk *fabsdk.FabricSDK) error {
